...
class MultiHeadDotProductAttention(nn.Module):
    ...
    def forward(self, x, keep_rate, mask=None):
        ...
        if keep_rate < 1:
            ...
            return x, index, idx, cls_attn, left_tokens, q, k, v

        return out, None, None, None, left_tokens, q, k, v
...
class Encoder1DBlock(nn.Module):
    ...
    def forward(self, inputs, keep_rate):
        ...
        x, index, idx, cls_attn, left_tokens, q, k, v = self.attention(x, keep_rate)
        ...
        y = self.mlp(y)
        return x + y, left_tokens, idx, q, k, v
...
class Encoder(nn.Module):
    ...
    def forward(self, img, mask=None):
        ...
        for i, layer in enumerate(self.layers):
            x, left_token, idx, q, k, v = layer[0](x, self.keep_rate[i])
        ...
        return self.encoder_norm(x), left_tokens, idxs, Q, K, V
...
class ViTPatch(nn.Module):
    ...
    def forward(self, img, mask=None):
        ...
        x = self.dropout(x)
        x, left_tokens, idxs, Q, K, V = self.transformer(x)

        # x1 = self.to_cls_token(x[:, 0])
        # return x1, x[:, 1:]
        return x, left_tokens, idxs, Q, K, V
...
class Self_Attention(nn.Module):
    def __init__(self, d_model=768, cls_number=100, pretrained=True):
        ...
        if pretrained:
            checkpoint = torch.load("./transformer/sam_ViT-B_16.pth")
    ...
    def forward(self, x):
        sa_fea, left_tokens, idxs, Q, K, V = self.model(x)
        return sa_fea, left_tokens, idxs, Q, K, V
